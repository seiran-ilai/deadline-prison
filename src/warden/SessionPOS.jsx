...
export default function SessionPOS({ session, inmates, setMsg, onPosChange }) {
  ...
  function currentLine() {
    ...
    switch (pick) {
      case 'signup': {
        ...
        return { line: { item_type: 'signup', person_name: cust, supervise: d.supervise, target_guard_id: d.supervise ? d.target_guard_id : null, amount: PRICE.signup + (d.supervise ? PRICE.supervise : 0) } }
      }
      case 'visit': {
  ...
  async function checkout() {
    ...
    if (cust && cart.some(c => c.item_type === 'signup') && !rosterNames.includes(cust)) {
      await supabase.rpc('warden_add_walkin', { p_session: sid, p_name: cust })
        ...
        <div className="body">
          ...
          {!pick ? <p className="empty">選一個品項以填寫細節</p> : (
            <div className="pos-detail">
              {pick === 'signup' && (<>
                <label className="pos-chk"><input type="checkbox" checked={d.supervise} onChange={e => setD({ ...d, supervise: e.target.checked, target_guard_id: '' })} />指定獄卒監督 +10 萬</label>
                {d.supervise && (
