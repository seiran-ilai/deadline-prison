...
export default function SessionTab({ currentSession, setCurrentSession, sessions, inmates, isWarden, setMsg, reloadShared }) {
  ...
  async function startTimer() {
    const { error } = await supabase.from('sessions')
      .update({ timer_started_at: new Date().toISOString(), timer_ended_at: null }).eq('id', currentSession)
  ...
  async function endTimer() {
    ...
    const { error } = await supabase.from('sessions')
      .update({ timer_ended_at: new Date().toISOString() }).eq('id', currentSession)
  ...
  async function resetTimer() {
    ...
    const { error } = await supabase.from('sessions')
      .update({ timer_started_at: null, timer_ended_at: null }).eq('id', currentSession)
  ...
  async function changeRounds(delta) {
    const cur = sessions.find(s => s.id === currentSession)
    if (!cur) return
    const next = (cur.total_rounds ?? 8) + delta
    let floor = 1
    if (cur.timer_started_at && !cur.timer_ended_at) {
      const elapsed = Math.floor((Date.now() - new Date(cur.timer_started_at).getTime()) / 1000)
      floor = Math.max(1, pomodoroState(elapsed, cur.total_rounds ?? 8).round)
    }
    if (next < floor) { setMsg('不能少於目前進行中的輪數'); return }
    const { error } = await supabase.from('sessions').update({ total_rounds: next }).eq('id', currentSession)
  ...
  const totalRounds = currentSessionObj?.total_rounds ?? 8
  let timerStatus = 'idle'   // idle | running | ended
  let curRound = 1
  if (currentSessionObj?.timer_started_at) {
    const elapsed = Math.floor((Date.now() - new Date(currentSessionObj.timer_started_at).getTime()) / 1000)
    const st = pomodoroState(elapsed, totalRounds, currentSessionObj.timer_ended_at)
    curRound = st.round
